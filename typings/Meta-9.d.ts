...
declare namespace imports.gi.Meta {
	...
	interface IBackend {
		...
		connect(signal: "prepare-shutdown", callback: (owner: this) => void): number;
	...
	interface IBackgroundContent {
		...
		/**
		 * Sets the bounding clip rectangle of the {@link BackgroundContent} that's used
		 * when a rounded clip set via {@link Meta.BackgroundContent.set_rounded_clip_radius}
		...
		connect(signal: "notify::meta-display", callback: (owner: this, ...args: any) => void): number;
		connect(signal: "notify::monitor", callback: (owner: this, ...args: any) => void): number;
		connect(signal: "notify::rounded-clip-radius", callback: (owner: this, ...args: any) => void): number;
	...
	interface IContext {
		...
		unsafe_mode: boolean;
		/**
		 * See {@link G.option_context_add_main_entries} for more details.
		...
		/**
		 * See {@link G.option_context_add_group} for more details.
		...
		set_plugin_gtype(plugin_gtype: GObject.Type): void;
		...
		connect(signal: "notify::unsafe-mode", callback: (owner: this, ...args: any) => void): number;
	...
	interface ICursorTracker {
		...
		connect(signal: "position-invalidated", callback: (owner: this) => void): number;
	...
	interface IDisplay {
		...
		connect(signal: "grab-op-begin", callback: (owner: this, object: Window, p0: GrabOp) => void): number;
		connect(signal: "grab-op-end", callback: (owner: this, object: Window, p0: GrabOp) => void): number;
		...
		connect(signal: "notify::compositor-modifiers", callback: (owner: this, ...args: any) => void): number;
	...
	interface IStartupSequence {
		...
		connect(signal: "timeout", callback: (owner: this) => void): number;
